...
export function DraftHeader({
  ...
  const renderPickSlots = (picks: string[], team: "blue" | "red") => {
    const slots = Array.from({ length: 5 }, (_, i) => {
      ...
      return (
        <div
          key={i}
          className={`w-24 h-100 sm:w-28 sm:h-100 md:w-32 md:h-100 lg:w-36 lg:h-100 xl:w-100 xl:h-100 rounded border-2 flex flex-col relative overflow-hidden transition-all duration-300 ${
            isActiveSlot
  ...
  return (
    <div className="lol-bg-darker border-b border-gray-700 px-6 py-4">
      {/* Progress Bar */}
      <div
        className={`w-full h-3 bg-gray-900 mb-4 flex rounded-full overflow-hidden shadow-inner ${isBlueTeamTurn ? "justify-start" : "justify-end"}`}
      >
        <div
          className={`h-full transition-all duration-1000 ease-linear ${progressBarColor} relative rounded-full`}
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-4 sm:gap-6 lg:gap-8">
          ...
          <div className="flex-1 w-full">
            ...
            <div className="mt-3 sm:mt-4">
              <h3 className="text-xs sm:text-sm font-medium lol-text-gray mb-1 sm:mb-2">
                BANLAR
              </h3>
          ...
          <div className="text-center px-2 sm:px-4 order-first lg:order-none">
            <div
              className="text-xl sm:text-2xl lg:text-3xl font-bold lol-text-gold"
